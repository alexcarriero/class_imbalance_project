...
# x <- readRDS("./../visualize_results/results/results_sc13_new.RData")$summary %>% prepare_for_results_table()
# y <- readRDS("./../visualize_results/results/results_sc14_new.RData")$summary %>% prepare_for_results_table()
# z <- readRDS("./../visualize_results/results/results_sc15_new.RData")$summary %>% prepare_for_results_table()
...
d <- as.matrix(d)
# x <- data.frame(rnames = rep(c("C", "SD", "Brier Score", "SD", "Cal Int", "SD", "Cal Slp", "SD"),6))
x <- data.frame(rnames = rep(c("Concordance Statistic", "MCMC Error", "Brier Score", "MCMC Error",
                               "Calibration Intercept", "MCMC Error", "Calibration Slope", "MCMC Error"),3))
rownames(d) <- x$rnames
...
d %>%
  kable(booktabs = T, format = "latex", align = rep("c", 30),
        ...
        linesep = c("\\addlinespace", "\\addlinespace", "\\addlinespace", "\\addlinespace", "\\addlinespace", "\\addlinespace", "\\addlinespace",
                  "\\addlinespace \\addlinespace \\addlinespace \\addlinespace"),
        caption = "Results Table", label = "tab:table") %>%
  kable_styling(latex_options = "scale_down") %>%
  kableExtra::landscape() %>%
  pack_rows("8 Predictors,  Event Fraction = 0.5",   1,  8) %>%
  pack_rows("8 Predictors,  Event Fraction = 0.2",   9, 16) %>%
  pack_rows("8 Predictors,  Event Fraction = 0.02", 17, 24) %>%
  # pack_rows("16 Predictors, Event Fraction = 0.5",  25, 32) %>%
  # pack_rows("16 Predictors, Event Fraction = 0.2",  33, 40) %>%
  # pack_rows("16 Predictors, Event Fraction = 0.02", 41, 48) %>%
  # add_indent(c(1:48), level_of_indent = 2) %>%
  add_indent(c(1:24), level_of_indent = 2) %>%
...
lr %>% 
  kable(booktabs = T, escape = F, linesep = " ", align = rep("c", 9), caption = "Errors: Logistic Regression", label = "tab:table") %>%
  kable_styling(full_width = F, latex_options ="hold_position")

sv %>% 
  kable(booktabs = T, escape = F, linesep = " ", align = rep("c", 9), caption = "Errors: Support Vector Machine", label = "tab:table") %>%
  kable_styling(full_width = F, latex_options ="hold_position")

rf %>%
  kable(booktabs = T, escape = F, linesep = " ", align = rep("c", 9), caption = "Errors: Random Forest", label = "tab:table") %>%
  kable_styling(full_width = F, latex_options ="hold_position")

xg %>% 
  kable(booktabs = T, escape = F, linesep = " ", align = rep("c", 9), caption = "Errors: XGBoost", label = "tab:table") %>%
  kable_styling(full_width = F, latex_options ="hold_position")

rb %>% 
  kable(booktabs = T, escape = F, linesep = " ", align = rep("c", 9), caption = "Errors: RUSBoost", label = "tab:table") %>%
  kable_styling(full_width = F, latex_options ="hold_position")

ee %>% 
  kable(booktabs = T, escape = F, linesep = " ", align = rep("c", 9), caption = "Errors: EasyEnsemble", label = "tab:table") %>%
  kable_styling(full_width = F, latex_options ="hold_position")
```
