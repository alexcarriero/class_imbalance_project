...
Errors and warnings were carefully monitored throughout our simulation.  We used the tryCatch.W.E function from `simsalapar` to save all warning and error messages [@simsalapar].  These messages are stored in a data file for each scenario and, along with the raw simulation results, are available upon request.  In Table X and Table Y we present the frequency with which errors occurred for the imbalance corrections and classification algorithms, respectively.\
\
...
df %>% 
  ...
  group_by(pair_id) %>% 
  group_modify(~non_zeros(.x$invalid_1)) %>%
  merge(pairs, by = "pair_id")
```
...
```{r}
count_invalid_probabilities <- function(dataframe){
  
}
```

```{r}
invalid_probs <- function(a,b) {
