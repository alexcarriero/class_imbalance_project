...
Conversely, when data were extremely imbalanced ($EF = 0.02$) and sample size is small ($\frac{1}{2}N$ some classification algorithms may return an error. In this case, predicted probabilties were stored as "$NA$".\

In our simulation, we save all warnings and errors, and report the frequency with which errors occur for each imbalance correction and algorithm. We used the tryCatch.W.E function from simsalapar to save errors and warnings in our simulation [@simsalapar]. 