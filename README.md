...
Please note, due to size contstraints, this repository does not contain all output from our simulation, only the processed results necessary for the creation of our Shiny App and project manuscript are present. Instead, we demonstrate the file structure necessary to exactly replicate the simulation study by including complete simulation output and processed results for one iteration of one simulation scenario (iteration 1 from simulation scenario 1).  This is accompanied by full instructions for how to populate this file structure with all simulation results after replicating our simulation study (more details below).
...
| [`manuscript`](./manuscript)     |  In this folder, you will find all necessary files to generate both our [`manuscript`](./manuscript/sim-manuscript/sim-manuscript.pdf) and [`supplementary materials`](./manuscript/supplementary_mat.Rmd). <br> This folder contains a README file with further details. |           
